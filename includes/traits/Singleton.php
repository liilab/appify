...
trait Singleton {
	...
	final public static function get_instance() {
		...
		if ( ! isset( $instance[ $called_class ] ) ) {
			...
			/**
			 * Dependent items can use the `wta_singleton_init_{$called_class}` hook to execute code
			 */
			do_action( sprintf( 'wta_singleton_init_%s', $called_class ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
